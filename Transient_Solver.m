for t=1:Time_steps
    ...
    while iter<max_iter/2 && residue>min_residue
        ...
        if Van_Driest_Damping_on == 1
            ...
            elseif bcswitch == 3
                Van_Driest_function = (1-exp(-abs(y_plus_U)/Van_Driest_A));
            end
        ...
        dpdx = unsteady_dpdx(t,Delta_t)*dpdx*(unsteady_function==0)...
            + dpdx*(unsteady_function==1) + (Qnew-Q)/Q*0.01*(prescribeswitch == 1);
        ...
        for k = 2+(wallfunction == 1):Nz+1; %if wall function used, start at 3d cell.
            ...
            u(k,t+1) = (ad*(tf*u(k-1,t+1)+(1-tf)*u(k-1,t)) ...          %down cells
                        ...
                        -1/rho*dpdx(k,t)*dz(k))/ap;                     %pressure term
        end
